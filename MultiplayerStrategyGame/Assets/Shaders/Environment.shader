Shader "Custom/Environment" {
	...
	SubShader {
		...
		void vert (inout appdata_full v) {
			...
			half height = tex2Dlod(_HeightMap, float4(worldPos.xz * _MaskSize, 0, 0)).r * _HeightStrength;
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			fixed4 t = lerp(tex2D (_Texture1, IN.uv_Texture1), tex2D (_Texture2, IN.uv_Texture2), tex2D (_TextureMask, IN.worldPos.xz * _MaskSize));
			fixed4 n = lerp(tex2D (_Normal1, IN.uv_Texture1), tex2D (_Normal2, IN.uv_Texture2), tex2D (_TextureMask, IN.worldPos.xz * _MaskSize));
